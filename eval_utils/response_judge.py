...
class Grade_LONG(BaseModel):
    is_correct: str = Field(description='yes or no')
    explanation: str = Field(description='Explain why the answer is yes or no.')
...
async def locomo_grader(llm_client, question: str, gold_answer: str, response: str) -> tuple[bool, str]:
    system_prompt = """
    ...
    **Furthermore, due to potential timezone differences in the data, if a specific date in the generated answer is off by exactly one day from the gold answer (either the day before or the day after), you should also consider it CORRECT.**
    ...
    try:
        ...
        # print(f"Input use {response.usage.total_tokens} tokens")
        return result.is_correct.strip().lower() == 'correct', result.explanation
    except openai.BadRequestError as e:
        if "connection reset by peer" in str(e):
            raise
        else:
            print(f"请求参数错误，不重试：{e}")
            return False, ""
...
async def locomo_response_timeline(llm_client, memories: List[str], user_profile: Any, question: str,
                                   timeline_memory_str: str) -> str:
    system_prompt = """
    ...
    You also have access to a timeline of memories for a specific topic.

    # INSTRUCTIONS:
    1. Carefully analyze all provided memories and the timeline.
    2. Pay special attention to the timestamps to determine the answer
    3. If the question asks about a specific event or fact, look for direct evidence in the memories and timeline.
    ...
    Memory: (2023-03-15 07:33 PM) I went to the vet yesterday.
...
async def locomo_eval_doubao(doubao_client, question: str, gold_answer: str, response: str) -> bool:
    system_prompt = """
        You are an expert grader that determines if answers to questions match a gold standard answer
        """
    ...
    label_value = response_content.get('label')
    return label_value.strip().lower() == 'correct'
